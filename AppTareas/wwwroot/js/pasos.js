function clickAgregarPasos() {
    ...
    tareaEditarVM.pasos.push(new pasoViewModel({ modoEdit: true, realizado: false }));

    $("[name=txtPasoDescripcion]:visible").focus();
}

function ClickCancelarPaso(paso) {
    if (paso.esNuevo()) {
...
async function ClickSalvarPaso(paso) {
    paso.modoEdit(false);
...
function ClickDescripcionPaso(paso) {
    paso.modoEdit(true);
...
function ClickCheckboxPaso(paso) {
