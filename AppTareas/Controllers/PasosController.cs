...
namespace AppTareas.Controllers
{
    ...
    public class PasosController : ControllerBase
    {
        ...
        public async Task<ActionResult<Paso>> Post(int tareaId, [FromBody] PasosAgregarDto dto)
        {
            ...
            paso.Descripcion = dto.Descripcion;
            paso.Realizado = dto.Realizado;
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Put(Guid id, [FromBody] PasosAgregarDto dto)
        {
            var usuarioId = _servicioUsuarios.ObtenerUsuarioId();
        ...
        public async Task<IActionResult> Ordenar(int tareaId, [FromBody] Guid[] ids)
        {
            ...
            var idsPasosNoPertenecen = ids.Except(pasosIds).ToList();

            if (idsPasosNoPertenecen.Any())
            {
                return BadRequest("No todos los pasos estan presentes");
            }

            var pasosDisccionario = pasos.ToDictionary(p => p.Id);

            for (int i = 0; i < ids.Length; i++)
            {
                var pasoId = ids[i];
                var paso = pasosDisccionario[pasoId];
                paso.Orden = i + 1;
            }
